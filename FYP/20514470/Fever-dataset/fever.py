...
# CONFIG
# Files 
ANSWER_CACHE_FILE = "fever_answers_cache19.csv"

OUT_JSON_FILE = "fever_paper_overall_summary19.json"
OUT_CSV_FILE  = "fever_paper_overall_summary19.csv"

ERROR_CASES_FILE = "fever_paper_error_cases19.csv"
MAX_ERROR_LOGS_PER_MODE = 200

MITI_CASES_FILE = "fever_mitigation_cases19.csv"
MAX_CASES_PER_RUN = 500

THRESHOLD_ANALYSIS_FILE = "fever_threshold_analysis19.csv"
...
# Safety valve: if still risky after rerank -> force abstain
...
# minimize: halluc_rate + 0.10*abstain_rate + 0.50*regress_rate
TUNE_W_ABSTAIN = 0.10
TUNE_W_REGRESS = 0.50
...
def clean_fever_output(text: str) -> str:
    """
    Clean model output by enforcing a strict FEVER format.
    """
    if not text:
        return ""

    label = parse_fever_label_from_answer(text)

    # fallback
    if label is None:
        label = "NOT ENOUGH INFO"

    # Extract reason (short and concise)
    reason = ""
...
def detector_mnli_signal(evidence: str, answer_text: str):
    label = parse_fever_label_from_answer(answer_text)
    if label is None:
        return 0.0, 0.0

    label_text = f"The claim is {label}."

    return nli_ent_con(det_tok, det_nli, evidence, label_text)
...
def gold_factual_ok(answer_text: str, evidence: str):
    ...
    con = gold_contradiction_prob(evidence, label_text)
...
def _extract_for_selfcheck(ans: str) -> str:
    if not ans:
        return ""

    label = parse_fever_label_from_answer(ans)
    if label is None:
        return ""

    return f"The claim is {label}."
...
def judge_truth(question: str, model_answer: str, gold_label: str, evidence: str) -> bool:
    """
    Binary judge (temperature 0) — stable enough; relying heavily on NLI+gold gate for correctness.
    """
    prompt = f"""
...
You are a FEVER fact-checking system.

Rules:
- If the evidence clearly supports the claim → SUPPORTED
- If the evidence clearly refutes the claim → REFUTED
- If the evidence is insufficient → NOT ENOUGH INFO
...
Rules:
- Use ONLY the Evidence (and copied spans if provided)
- DO NOT add ANY new facts
- If the evidence proves the claim, output SUPPORTED
- If the evidence disproves the claim, output REFUTED
- If the evidence is insufficient, output NOT ENOUGH INFO
...
def _filter_candidates(question, evidence, gold_label, candidates):
    ...
    for c in candidates:
        ...
        label = parse_fever_label_from_answer(c)
        if label is None:
            continue

        label_text = f"The claim is {label}."
        gcon = gold_contradiction_prob(evidence, label_text) if evidence.strip() else 0.0
...
def detector_conditioned_correct(
    ...
    for lbl, cands in groups.items():
        ...
        # use cands (already filtered & grouped) 
        for ai in cands:
    ...
    if abstain_if_still_risky and (best_ok < abstain_threshold):
        ...
        abstain_ok, _ = score_single_answer_full(
            question, evidence, gold_label, abstain_ans, sc_override=best_group_sc
        )

        if abstain_ok >= best_ok:
            return abstain_ans, abstain_ok, f"{action}_abstain", base_ok
...
def eval_on_packs(packs, mode, threshold, run_id=0, log_errors_path=None, max_log=0):
    ...
    for pack in packs:
        ...
        for a in pack["per_answer"]:
            ...
            prob_correct = None
            if mode == "full":
                prob_halluc = float(halluc_calibrator.predict([score])[0])
                prob_correct = float(1.0 - prob_halluc)
                halluc_probs.append(prob_halluc)
                correct_probs.append(prob_correct)
    ...
    return {
        ...
        "halluc_prob_mean": float(np.mean(halluc_probs)) if halluc_probs else 0.0,
        "correct_prob_mean": float(np.mean(correct_probs)) if correct_probs else 0.0
    }
...
def fit_hallucination_calibrator(val_packs, mode="full"):
    xs = []
    ys = []
    ...
    for pack in val_packs:
        ...
        for a in pack["per_answer"]:
            ...
            xs.append(float(s))
            ys.append(0 if a["gold_ok"] else 1)   # 1 = hallucination

    calibrator = IsotonicRegression(increasing=False, out_of_bounds="clip")
...
def eval_mitigation_on_packs(
    ...
    for pack in packs:
        ...
        base_prob_halluc = float(halluc_calibrator.predict([base_ok])[0])
        final_prob_halluc = float(halluc_calibrator.predict([final_ok])[0])

        base_prob_correct = 1.0 - base_prob_halluc
        final_prob_correct = 1.0 - final_prob_halluc

        base_halluc_probs.append(base_prob_halluc)
        final_halluc_probs.append(final_prob_halluc)
        base_correct_probs.append(base_prob_correct)
        final_correct_probs.append(final_prob_correct)
        ...
        if base_gold_ok and (not final_gold_ok):
            regress_cnt += 1
        if (not base_gold_ok) and abstain:
    ...
    return {
        ...
        "base_halluc_prob_mean": float(np.mean(base_halluc_probs)) if base_halluc_probs else 0.0,
        "final_halluc_prob_mean": float(np.mean(final_halluc_probs)) if final_halluc_probs else 0.0,
        "delta_halluc_prob_mean": float(np.mean(np.array(final_halluc_probs) - np.array(base_halluc_probs))) if base_halluc_probs else 0.0,

        "base_correct_prob_mean": float(np.mean(base_correct_probs)) if base_correct_probs else 0.0,
        "final_correct_prob_mean": float(np.mean(final_correct_probs)) if final_correct_probs else 0.0,
        "delta_correct_prob_mean": float(np.mean(np.array(final_correct_probs) - np.array(base_correct_probs))) if base_correct_probs else 0.0,
...
print("\nFitting hallucination probability calibrator on VAL...")
halluc_calibrator = fit_hallucination_calibrator(val_packs)
print("  calibration model fitted (IsotonicRegression)")

# Calibration sanity check 
test_scores = np.linspace(0, 1, 20)
test_probs = halluc_calibrator.predict(test_scores)

print("\nCalibration check:")
for s, p in zip(test_scores, test_probs):
    print(f"score={s:.2f} -> P(halluc)={p:.3f}")
...
# OPTIONAL: tune t_high on VAL to minimize hallucination 
...
def tune_t_high_on_val(val_packs_for_tune, t_low_ok, K, abstain_threshold):
    ...
    # run only "policy" on a small val subset to tune t_high
...
threshold_grid = np.linspace(t_low_ok, min(0.95, t_low_ok + 0.25), 10)

threshold_analysis = []

for th in threshold_grid:
    stats = eval_mitigation_on_packs(
        val_packs,
        policy_name="policy",
        t_low_ok=t_low_ok,
        t_high_ok=th,
    ...
    threshold_analysis.append({
        "t_high": float(th),
        "halluc_rate": float(stats["final_halluc_rate"]),
        "abstain_rate": float(stats["abstain_rate"]),
        "regress_rate": float(stats["paired"]["regress_cnt"] / stats["n_questions"])
    })

# Save CSV 
with open(THRESHOLD_ANALYSIS_FILE, "w", newline="", encoding="utf-8") as f:
...
for m in modes:
    ...
    halluc_probs = [r["halluc_prob_mean"] for r in summary[m]]
    correct_probs = [r["correct_prob_mean"] for r in summary[m]]
    ...
    print(
        f"{m:12s} | "
        f"F1={safe_mean(f1s):.4f}±{safe_std(f1s):.4f} | "
        f"Q-std={safe_mean(qstds):.4f} | "
        f"A-std={safe_mean(astds):.4f} | "
        f"hall_p={safe_mean(halluc_probs):.3f} | "
        f"corr_p={safe_mean(correct_probs):.3f} | "
        f"thr={mode_thresholds[m]:.2f}"
    )

print("\n=== MITIGATION SUMMARY (Question-level final answer) ===")
print(
    f"{'policy':16s} | "
    f"{'gold_ok':>10s} {'halluc':>10s} | "
    f"{'filtered_halluc':>14s} {'valid_ans':>10s} | "
    f"{'NEI':>8s} {'abstain':>10s} | "
    f"{'Δok':>10s} | "
    f"{'impr/reg/abst':>18s} | "
    f"{'hall_p(base->final)':>22s} | "
    f"{'corr_p(base->final)':>22s}"
)
print("-" * 120)
...
for p in ["base", "always_mild", "always_strict_K", "policy"]:
    ...
    halluc_prob_before = [x["base_halluc_prob_mean"] for x in miti_summary[p]]
    halluc_prob_after  = [x["final_halluc_prob_mean"] for x in miti_summary[p]]

    correct_prob_before = [x["base_correct_prob_mean"] for x in miti_summary[p]]
    correct_prob_after  = [x["final_correct_prob_mean"] for x in miti_summary[p]]   
    ...
    summary_json["mitigation"][p] = {
        ...
        "filtered_halluc_rate_mean": safe_mean([x["filtered_halluc_rate"] for x in miti_summary[p]]),
        "filtered_halluc_rate_std": safe_std([x["filtered_halluc_rate"] for x in miti_summary[p]]),

        "valid_answer_rate_mean": safe_mean([x["valid_answer_rate"] for x in miti_summary[p]]),
        "valid_answer_rate_std": safe_std([x["valid_answer_rate"] for x in miti_summary[p]]),    
        "base_halluc_prob_mean": safe_mean(halluc_prob_before),
        "final_halluc_prob_mean": safe_mean(halluc_prob_after),
        "base_correct_prob_mean": safe_mean(correct_prob_before),
        "final_correct_prob_mean": safe_mean(correct_prob_after),       
    ...
    print(
        ...
        f"{safe_mean(filtered_halluc_rates):14.4f} "
        f"{safe_mean(valid_rates):10.4f} | "
        
        f"{safe_mean(nei_rates):8.4f} {safe_mean(abstain_rates):10.4f} | "
        
        f"{safe_mean(deltas):10.4f} | "
        
        f"{safe_mean(impr):6.2f}±{safe_std(impr):.2f}/"
        f"{safe_mean(reg):6.2f}±{safe_std(reg):.2f}/"
        f"{safe_mean(abst):6.2f}±{safe_std(abst):.2f} | "
        
        f"{safe_mean(halluc_prob_before):.3f}->{safe_mean(halluc_prob_after):.3f} | "
        f"{safe_mean(correct_prob_before):.3f}->{safe_mean(correct_prob_after):.3f}"
    )
