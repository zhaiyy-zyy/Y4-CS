...
# ---------- Files ----------
ANSWER_CACHE_FILE = "nq_answers_cache_final_all8.csv"

OUT_JSON_FILE = "nq_paper_overall_summary_final_all8.json"
OUT_CSV_FILE = "nq_paper_overall_summary_final_all8.csv"

ERROR_CASES_FILE = "nq_paper_error_cases_final_all8.csv"
MAX_ERROR_LOGS_PER_MODE = 200

MITI_CASES_FILE = "nq_mitigation_cases_final_all8.csv"
...
def score_single_answer_mitigation(question, context, answer_text, sc_override=None):
    answer_for_eval = parse_final_answer_text(answer_text)
    ent, con = detector_mnli_signal(context, answer_for_eval)
    sc = float(sc_override) if sc_override is not None else 0.0

    fact_score = fact_score_from_ent_con(ent, con)
    sc_score = 1.0 - sc

    ok_score = 0.75 * fact_score + 0.25 * sc_score
    return ok_score, {
        "ent": float(ent),
        "con": float(con),
        "sc": float(sc)
...
# Mitigation
# ============================================================
def build_mild_rewrite_prompt(question, prev_answer, context):
    return f"""
Question: {question}
...
def build_strict_rewrite_prompt(question, prev_answer, context):
    return f"""
...
Task:
- If incorrect → fix using context
- If correct → keep
...
def rewrite_once(question, prev_answer, kind="mild", context=""):
    if kind == "mild":
        prompt = build_mild_rewrite_prompt(question, prev_answer, context)
        temp = MILD_TEMP
    else:
        prompt = build_strict_rewrite_prompt(question, prev_answer, context)
        temp = STRICT_TEMP
...
def _filter_candidates(question, context, candidates):
    kept = []
    for c in candidates:
        ...
        ent, con = detector_mnli_signal(context, ans)  # 用 context
        ...
        ans_low = ans.lower()
        is_abstain = any(x in ans_low for x in [
            "cannot determine", "insufficient", "do not know", "don't know", "not enough information"
        ])
...
def detector_conditioned_correct(
    ...
    base_ok, _ = score_single_answer_mitigation(
        question, context, base_answer, sc_override=None
    )
    ...
    if base_ok < 0.15:
        ...
        abst_ok, _ = score_single_answer_mitigation(
            question, context, abst_ans, sc_override=None
        )
    ...
    def gen_candidates(K_local, context):
        ...
        for j in range(int(K_local)):
            if j < K_local // 3:
                ai = rewrite_once(question, base_answer, kind="mild", context=context)
            elif j < 2 * K_local // 3:
                ai = rewrite_once(question, base_answer, kind="strict", context=context)
            else:
                ai = generate_grounded_answer(question, context)
    ...
    filtered = _filter_candidates(question, context, raw_cands)
    if not filtered:
        filtered = raw_cands
    ...
    for lbl, cands in groups.items():
        ...
        base_ok_sc, _ = score_single_answer_mitigation(
            question, context, base_answer, sc_override=sc_grp
        )
        ...
        for ai in cands:
            oki, _ = score_single_answer_mitigation(
                question, context, ai, sc_override=sc_grp
            )
    ...
    if (abstain_if_still_risky and (best_ok < abstain_threshold) and (best_ok < base_ok + ABSTAIN_ADVANTAGE)):
        abst_ans = "FINAL_ANSWER: I cannot determine the answer."
        abst_ok, _ = score_single_answer_mitigation(
            question, context, abst_ans, sc_override=best_group_sc
        )
...
def always_rewrite_correct(question, reference, ref_sentence, base_answer, context="", kind="mild", K=1):
    if kind == "mild":
        ai = rewrite_once(question, base_answer, kind="mild", context=context)  
        if not ai:
            ok0, _ = score_single_answer_mitigation(
                question, context, base_answer, sc_override=None
            )
            return base_answer, ok0

        ok1, _ = score_single_answer_mitigation(
            question, context, ai, sc_override=None
        )
        return ai, ok1
    ...
    filtered = _filter_candidates(question, context, raw)  
    if not filtered:
        filtered = raw
...
# Mitigation eval
# ============================================================
def tune_mitigation_thresholds(val_packs):
    scores = []
    golds = []
    ...
    for pack in val_packs:
        ...
        score, _ = score_single_answer_mitigation(
            q, context, base_clean, sc_override=None
        )

        gold_ok = nq_gold_ok(base_clean, ref)

        scores.append(score)
        golds.append(gold_ok)
...
def eval_mitigation_on_packs(
    ...
    for pack in packs:
        ...
        base_ok, _ = score_single_answer_mitigation(q, context, base_answer, sc_override=None)
