...
# ---------- Files ----------
ANSWER_CACHE_FILE = "truthfulqa_answers_cache_all9.csv"

OUT_JSON_FILE = "truthfulqa_paper_overall_summary_all9.json"
OUT_CSV_FILE  = "truthfulqa_paper_overall_summary_all9.csv"

ERROR_CASES_FILE = "truthfulqa_paper_error_cases_all9.csv"
MAX_ERROR_LOGS_PER_MODE = 200

MITI_CASES_FILE = "truthfulqa_mitigation_cases_all9.csv"
...
def tqa_gold_ok(answer, correct_answers, incorrect_answers, best_answer=None):
    ...
    # ---------- negative condition ----------
    # 注意：这里只看 answer 是否“贴近错误答案”
    # 不再把 contradiction to incorrect 当坏事
    negative_match = (
...
def score_single_answer_real(question, answer_text, peer_answers=None, sc_override=None):
    ...
    # mitigation内部评分：偏保守
    judge_score = 1.0 if judge_ok else 0.0
    peer_score = max(ent - 0.8 * con, 0.0)
    sc_score = 1.0 - sc

    ok_score = 0.45 * judge_score + 0.35 * peer_score + 0.20 * sc_score
...
def build_question_pack(q_idx):
    ...
    for i, item in enumerate(per_answer):
        s = mode_score(
            ...
            good_ent=item["good_ent"],
            bad_ent=item["bad_ent"],
            good_contra=item["good_contra"],
