...
# ---------- Files ----------
ANSWER_CACHE_FILE = "nq_answers_cache_final_all22.csv"

OUT_JSON_FILE = "nq_paper_overall_summary_final_all22.json"
OUT_CSV_FILE = "nq_paper_overall_summary_final_all22.csv"

ERROR_CASES_FILE = "nq_paper_error_cases_final_all22.csv"
MAX_ERROR_LOGS_PER_MODE = 200

MITI_CASES_FILE = "nq_mitigation_cases_final_all22.csv"
...
def _filter_candidates(question, reference, ref_sentence, candidates, context):
    ...
    for c in candidates:
        ...
        if not is_span_in_context(ans, context):
            continue
...
def detector_conditioned_correct(
    ...
    for lbl, cands in groups.items():
        ...
        for ai in cands:
            # ⭐ 每个 answer 自己算 consistency
            sc_i = selfcheck_one_answer(ai, [x for x in cands if x != ai])

            if nq_gold_ok(ai, reference):
                return final_clean_output(ai), 1.0, f"{tag}_gold_force_accept", base_ok

            oki, _ = score_single_answer_full(
                question, reference, ref_sentence, ai, sc_override=sc_i
            )
