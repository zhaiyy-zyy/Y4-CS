...
# ---------- Files ----------
ANSWER_CACHE_FILE = "truthfulqa_answers_cache_all17.csv"

OUT_JSON_FILE = "truthfulqa_paper_overall_summary_all17.json"
OUT_CSV_FILE  = "truthfulqa_paper_overall_summary_all17.csv"

ERROR_CASES_FILE = "truthfulqa_paper_error_cases_all17.csv"
MAX_ERROR_LOGS_PER_MODE = 200

MITI_CASES_FILE = "truthfulqa_mitigation_cases_all17.csv"
...
def score_single_answer_real(question, answer_text, peer_answers=None, sc_override=None, mode="full"):
    ...
    judge_score = 0.7 if judge_ok else 0.0

    # -------- Peer consistency (NLI-based, non-gold) --------
    ...
    # ===== NEW: FEVER-style contradiction gate =====

    # base risk
    halluc_risk = con

    # exponential penalty (关键)
    contra_penalty = np.exp(FACT_CONTRA_ALPHA * con) - 1.0

    # normalize to [0,1] range
    contra_penalty = min(contra_penalty, 3.0) / 3.0

    # combine
    halluc_risk = halluc_risk + contra_penalty

    # -------- TQA detector has NO gold grounding --------
    ground_score = max(ent - con, 0)
    lexical_score = 0.0

    # -------- mode-controlled ablation --------
    if mode == "full":
        use_judge = 1.0
        use_peer = 1.0
        use_sc = 1.0
        ...
        # remove peer-NLI and NLI-based penalty
        use_judge = 1.0
        use_peer = 0.0
        use_sc = 1.0
        use_penalty = 0.0
    ...
    ok_score = (
        use_judge * W_JUDGE * judge_score
        + use_peer * W_PEER * peer_score
        + use_sc * W_SC * sc_score
        + W_GROUND * ground_score
        + W_LEX * lexical_score
        - use_penalty * (W_PENALTY * halluc_risk + length_penalty)
    )
    # ===== 🔥 SOFT CONTRADICTION GATE（加这里）=====
    if use_penalty:   # ⚠️ 保证 no_mnli / judge_only 不受影响
        ok_score -= 0.4 * con

    # ===== 🔥 HARD CONTRADICTION GATE（加在这里！）=====
    if use_penalty:   # ⚠️ 很重要：no_mnli模式不能触发
        if con > 0.75:
            ok_score *= 0.3
        elif con > 0.60:
            ok_score *= 0.6
...
def train_calibrator(samples):
    ...
    for s in samples:
        ...
        _, meta = score_single_answer_real(
            q,
            a["answer"],
    ...
    print("\n[Calibration] trained on", len(X), "samples")
    return clf
...
def detector_conditioned_correct(
    ...
    if base_ok >= t_high_ok:
        ...
        features = [
            float(base_meta["judge_ok"]),
            base_meta["ent"],
            base_meta["con"],
            base_meta["sc"]
        ]

        base_p_hall = predict_halluc_prob(calibrator, features)
...
calibrator = train_calibrator(samples)
...
def eval_on_packs(packs, mode, threshold, run_id=0, log_errors_path=None, max_log=0):
    ...
    for pack in packs:
        ...
        for a in pack["per_answer"]:
            ...
            p_hall = predict_halluc_prob(calibrator, features)

            pred_ok = (p_hall < threshold)
            ...
            all_scores.append(p_hall)
            _, abstain = compute_nei_and_abstain(a["answer"])

            if not abstain:
                scores_this_q.append(p_hall)
...
def tune_threshold_on_val(val_packs, mode, grid=None):
    ...
    for pack in val_packs:
        ...
        for a in pack["per_answer"]:
            ...
            # ✅ 用 calibrated probability
            p_hall = predict_halluc_prob(calibrator, features)

            scores.append(p_hall)
            golds.append(a["gold_ok"])

    scores = np.array(scores, dtype=np.float32)
    golds = np.array(golds, dtype=np.bool_)
    gold_halluc = ~golds   # True = hallucination
...
def eval_mitigation_on_packs(
    ...
    for pack in packs:
        ...
        final_p_hall = predict_halluc_prob(calibrator, features)
...
summary_json = {
    "config": {
        ...
        "calibration": {
            ...
            "features": ["judge_ok", "peer_ent", "peer_con", "selfcheck"]
        },
        "mitigation": {
            "MITI_K": MITI_K,
            "MID_KIND": MID_KIND,
