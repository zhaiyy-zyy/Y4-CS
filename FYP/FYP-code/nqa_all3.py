...
# ---------- Files ----------
ANSWER_CACHE_FILE = "nq_answers_cache_final_all3.csv"

OUT_JSON_FILE = "nq_paper_overall_summary_final_all3.json"
OUT_CSV_FILE = "nq_paper_overall_summary_final_all3.csv"

ERROR_CASES_FILE = "nq_paper_error_cases_final_all3.csv"
MAX_ERROR_LOGS_PER_MODE = 200

MITI_CASES_FILE = "nq_mitigation_cases_final_all3.csv"
...
def always_rewrite_correct(question, reference, base_answer, kind="mild", K=1):
    if kind == "mild":
        ...
        if not ai:
            ok0, _ = score_single_answer_full(
                question, reference, base_answer, sc_override=None
            )
            return base_answer, ok0

        ok1, _ = score_single_answer_full(
            question, reference, ai, sc_override=None
        )
        return ai, ok1
    ...
    for lbl, cands in groups.items():
        ...
        sc_grp = selfcheck_group(cands)
        base_ok_sc, _ = score_single_answer_full(
            question, reference, base_answer, sc_override=sc_grp
        )
        ...
        for ai in cands:
            oki, _ = score_single_answer_full(
                question, reference, ai, sc_override=sc_grp
            )
            if oki > local_best_ok:
                local_best_ok = oki
                local_best_ans = ai
    ...
    for pack in packs:
        ...
        if policy_name == "base":
            ...
            final_answer, final_ok = always_rewrite_correct(
                q, ref, base_answer, kind="mild", K=1
            )
            action = "always_mild"
        elif policy_name == "always_strict_K":
            final_answer, final_ok = always_rewrite_correct(
                q, ref, base_answer, kind="strict", K=K
            )
        ...
        base_ent, base_con = detector_mnli_signal(ref, base_clean_answer)
        final_ent, final_con = detector_mnli_signal(ref, final_clean_answer)
