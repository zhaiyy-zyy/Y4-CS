...
# ---------- Files ----------
ANSWER_CACHE_FILE = "nq_answers_cache_final_all16.csv"

OUT_JSON_FILE = "nq_paper_overall_summary_final_all16.json"
OUT_CSV_FILE = "nq_paper_overall_summary_final_all16.csv"

ERROR_CASES_FILE = "nq_paper_error_cases_final_all16.csv"
MAX_ERROR_LOGS_PER_MODE = 200

MITI_CASES_FILE = "nq_mitigation_cases_final_all16.csv"
...
def is_abstain_answer(ans: str):
    ...
    return (
        x == ""
        or x == "not_found"
        or "cannot determine" in x
        or "not enough information" in x
        or "insufficient information" in x
    )


def compute_nei_and_abstain(answer_text: str):
    ans = final_clean_output(answer_text).lower()
...
def nq_gold_ok(answer, reference):
    ...
    for r in refs:
        ...
        # ⭐ 3. relaxed token match（从0.8 → 0.5）
        if token_f1(answer_norm, r) >= 0.5:
            return True
...
- If no reliable answer span is found in the Context, output exactly: NOT_FOUND
...
def _filter_candidates(question, reference, ref_sentence, candidates, context):
    ...
    for c in candidates:
        ...
        if (not is_abstain) and ent < 0.25:
            continue
...
def _group_by_label(candidates):
    ...
    for c in candidates:
        ...
        if is_abstain_answer(ans):
            groups["ABSTAIN"].append(ans)
        else:
            groups["ANSWER"].append(ans)
...
def detector_conditioned_correct(
    ...
    if base_ok < 0.15:
        ...
        forced = generate_candidate_extractive_answer(question, context)
        forced = final_clean_output(forced)

        if forced and (not is_bad_answer(forced)):
            okf, _ = score_single_answer_full(
                question, reference, ref_sentence, forced, sc_override=None
            )
            return forced, okf, "force_extract_low_conf", base_ok
    ...
    if abstain_if_still_risky and (best_ok < abstain_threshold):
        ...
        # 只有当 abstain 不明显更差时才触发
        if abst_ok + ABSTAIN_PENALTY >= best_ok:
