...
# ---------- Files ----------
ANSWER_CACHE_FILE = "nq_answers_cache_final_all13.csv"

OUT_JSON_FILE = "nq_paper_overall_summary_final_all13.json"
OUT_CSV_FILE = "nq_paper_overall_summary_final_all13.csv"

ERROR_CASES_FILE = "nq_paper_error_cases_final_all13.csv"
MAX_ERROR_LOGS_PER_MODE = 200

MITI_CASES_FILE = "nq_mitigation_cases_final_all13.csv"
...
def parse_final_answer_text(answer_text: str):
    ...
    # 去掉 content: 前缀（⭐关键修复）
    ans = re.sub(r'^\s*content\s*:\s*', '', ans, flags=re.I)
...
def final_clean_output(ans: str):
    ...
    # ⭐ NEW: treat "content:" as empty
    if re.fullmatch(r'\s*content\s*:?\s*', ans, flags=re.I):
        return ""
...
Rules:
- Copy ONLY the answer words from the Context
- Do NOT paraphrase
- Do NOT explain
- If the answer words are not present in the Context, output exactly:
NOT_FOUND
