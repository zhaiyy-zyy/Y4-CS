...
ABSTAIN_MARGIN = 0.12  # abstain_threshold = t_low_ok + margin
...
- Prefer using exact wording from the evidence.
...
def detector_conditioned_correct(
    ...
    if base_ok < (t_low_ok - 0.1):
        abst_ans = "LABEL: NOT ENOUGH INFO\nREASON: The evidence is insufficient."
        abst_ok, _ = score_single_answer_full(
            question, evidence, gold_label, abst_ans, sc_override=None
        )
    ...
    for lbl, cands in groups.items():
        ...
        for ai in cands:
            ...
            pred_lbl = parse_fever_label_from_answer(ai)
            if pred_lbl == "NOT ENOUGH INFO":
                oki -= 0.15
