...
# ---------- Files ----------
ANSWER_CACHE_FILE = "nq_answers_cache_final_all5.csv"

OUT_JSON_FILE = "nq_paper_overall_summary_final_all5.json"
OUT_CSV_FILE = "nq_paper_overall_summary_final_all5.csv"

ERROR_CASES_FILE = "nq_paper_error_cases_final_all5.csv"
MAX_ERROR_LOGS_PER_MODE = 200

MITI_CASES_FILE = "nq_mitigation_cases_final_all5.csv"
...
Rules:
- Use ONLY the reference answer and reference context.
- DO NOT add any new facts.
- Keep the answer short.
- Prefer copying from the reference or context.
...
def generate_grounded_answer(question, reference, context):
    prompt = f"""
Question:
{question}
...
Answer using ONLY the reference and context.
...
def _filter_candidates(question, reference, ref_sentence, candidates):
    """
    Hard filters to remove obviously bad candidates:
      1) non-empty
      2) parseable final answer
      3) weak-support + medium-contradiction candidates are removed
      4) strongly contradictory candidates under detector NLI are removed
      5) strongly contradictory candidates under GOLD NLI are removed
    """
    kept = []
    ...
    for c in candidates:
        ...
        # FEVER-style weak support + medium contradiction
        if ent < 0.2 and con > 0.4:
            continue

        # FEVER-style strong contradiction gate
        if con > 0.6:
            continue

        # require at least some support unless it's an abstain candidate
        ans_low = ans.lower()
        is_abstain = (
...
def detector_conditioned_correct(
    ...
    if base_ok < max(MIN_HARD_ABSTAIN_SCORE, t_low_ok - HARD_ABSTAIN_DELTA):
    ...
    def gen_candidates(K_local, context):
        ...
        for j in range(int(K_local)):
            if j < K_local // 3:
                ...
                ai = generate_grounded_answer(question, reference, context)
    ...
    filtered = _filter_candidates(question, reference, ref_sentence, raw_cands)
    if not filtered:
        filtered = raw_cands
...
def always_rewrite_correct(question, reference, ref_sentence, base_answer, context="", kind="mild", K=1):
    ...
    filtered = _filter_candidates(question, reference, ref_sentence, raw)
    if not filtered:
        filtered = raw
