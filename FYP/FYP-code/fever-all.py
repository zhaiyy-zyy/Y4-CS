...
# Adds:
#   - risk-conditioned re-prompting (t_low/t_high)
#   - strict K-sampling rerank by detector
#   - mitigation evaluation: gold_ok_rate, halluc_rate, NEI_rate, abstain_rate, Δok_score
# ============================================================
...
# -----------------------------
# Mitigation config (NEW)
# -----------------------------
# High-risk sampling count (K=3~5 recommended)
MITI_K = 5

# Temperature for rewrite calls (lower => more deterministic)
MILD_TEMP = 0.3
STRICT_TEMP = 0.2

# If you want abstain_rate to include empty/failed answers as abstain:
...
# -----------------------------
# Judge (Detector feature, NOT oracle)
# -----------------------------
...
def mode_score(sc, judge_ok, ent, con, mode):
    """
    Continuous ok_score in ~[0,1]. Threshold applied externally.
    """
...
def score_single_answer_full(question, evidence, gold_label, answer_text):
    """
    Single-answer detector ok_score for mitigation reranking.
    Note: selfcheck requires multiple answers; here we set sc=0.
    """
...
def detector_conditioned_correct(question, evidence, gold_label, base_answer,
    ...
    One-step closed-loop:
      - if ok >= t_high: keep
      - elif ok >= t_low: mild rewrite once, re-score, accept if not worse (else keep)
      - else: strict rewrite K candidates, re-score each, pick best ok_score
    Returns final_answer, final_ok_score, action
    """
    base_ok, _ = score_single_answer_full(question, evidence, gold_label, base_answer)
    ...
    if base_ok >= t_low_ok:
        ...
        ok1, _ = score_single_answer_full(question, evidence, gold_label, a1)
        if ok1 >= base_ok:
    ...
    for _ in range(K):
        ...
        if not ai:
            continue
        oki, _ = score_single_answer_full(question, evidence, gold_label, ai)
        if oki > best_ok:
            best_ok = oki
            best_ans = ai

    return best_ans, best_ok, f"strict_K{K}", base_ok

def always_rewrite_correct(question, evidence, gold_label, base_answer, kind="mild", K=1):
    """
    Baseline: always rewrite (no conditioning)
      - if K=1: rewrite once
      - if K>1: rewrite K candidates, pick best by detector ok_score
    """
    best_ans = base_answer
    best_ok, _ = score_single_answer_full(question, evidence, gold_label, base_answer)
...
def compute_nei_and_abstain(answer_text: str):
    """
    NEI rate: predicted label == NOT ENOUGH INFO
    abstain rate: default same as NEI, optionally count empty as abstain
    """
    ...
    abstain = nei  # define abstain as NEI by default
    return nei, abstain
...
# -----------------------------
# Evaluation helpers (original detection eval)
# -----------------------------
...
# -----------------------------
# Mitigation evaluation (NEW)
# -----------------------------
def eval_mitigation_on_packs(packs, policy_name, t_low_ok, t_high_ok, K=5):
    """
    Question-level final-answer evaluation:
      - choose a base answer per question
      - apply mitigation method
      - compute:
          final_gold_ok_rate, final_halluc_rate
          NEI_rate, abstain_rate
          ok_score before/after + delta
    """
    total = 0
    gold_ok_cnt = 0
    nei_cnt = 0
    ...
    for pack in packs:
        ...
        # Base answer choice: take first cached answer
        base_answer = pack["per_answer"][0]["answer"] if pack["per_answer"] else ""

        # base detector ok_score
        base_ok, _ = score_single_answer_full(q, ev, gl, base_answer)
...
# Define mitigation thresholds based on tuned full threshold (ok_score scale)
...
# -----------------------------
# Final summary + ΔF1 vs full (original)
# -----------------------------
