...
# ---------- Files ----------
ANSWER_CACHE_FILE = "truthfulqa_answers_cache_all3.csv"

OUT_JSON_FILE = "truthfulqa_paper_overall_summary_all3.json"
OUT_CSV_FILE  = "truthfulqa_paper_overall_summary_all3.csv"

ERROR_CASES_FILE = "truthfulqa_paper_error_cases_all3.csv"
MAX_ERROR_LOGS_PER_MODE = 200

MITI_CASES_FILE = "truthfulqa_mitigation_cases_all3.csv"
...
def tqa_gold_ok(answer, correct_answers, incorrect_answers, thresh=GOLD_THR):
    """
    Gold=True if:
...
Rules:
- Use only information supported by the reference truths.
- Prefer reusing the wording of the reference truths.
- Remove all unsupported, speculative, or extra content.
- Keep the answer as short as possible.
- Do not explain.
- Do not justify.
- Do not add background knowledge.
- If the answer cannot be safely corrected, output an abstention.
...
def _filter_candidates(question, correct, incorrect, candidates):
    kept = []
    for c in candidates:
...
def detector_conditioned_correct(
    question, correct, incorrect, base_answer,
    t_low_ok, t_high_ok,
    K=10,
    mid_kind="strict",
    abstain_if_still_risky=True,
    ...
    def gen_candidates(kind, K_local):
        ...
        for j in range(int(K_local)):
            if j < K_local // 2:
                # 原来的 rewrite
                ai = rewrite_once(question, correct, base_answer, kind=kind)
            else:
                # 新增 grounded answer
    ...
    filtered = _filter_candidates(question, correct, incorrect, raw_cands)
    if not filtered:
        filtered = raw_cands
    ...
    if abstain_if_still_risky and (base_ok < t_low_ok) and (best_ok < abstain_threshold):
...
def always_rewrite_correct(question, correct, incorrect, base_answer, kind="mild", K=1):
    if kind == "mild":
    ...
    filtered = _filter_candidates(question, correct, incorrect, raw)
    if not filtered:
        filtered = raw
...
def build_question_pack(q_idx):
    ...
    for ans in answers:
        ...
        gold_ok = tqa_gold_ok(ans_eval, correct, incorrect)
...
def eval_mitigation_on_packs(
    ...
    for pack in packs:
        ...
        base_gold_ok = tqa_gold_ok(parse_final_answer_text(base_answer), correct, incorrect)
        ...
        if policy_name == "base":
            ...
            final_answer, final_ok = always_rewrite_correct(q, correct, incorrect, base_answer, kind="mild", K=1)
            action = "always_mild"
        elif policy_name == "always_strict_K":
            final_answer, final_ok = always_rewrite_correct(q, correct, incorrect, base_answer, kind="strict", K=K)
            action = f"always_strict_K{K}_group_sc"
        elif policy_name == "policy":
            final_answer, final_ok, action, _ = detector_conditioned_correct(
                q, correct, incorrect, base_answer,
                t_low_ok=t_low_ok,
                t_high_ok=t_high_ok,
                K=K,
                mid_kind=MID_KIND,
        ...
        final_gold_ok = tqa_gold_ok(parse_final_answer_text(final_answer), correct, incorrect)
