...
# ---------- Files ----------
ANSWER_CACHE_FILE = "nq_answers_cache_final_all24.csv"

OUT_JSON_FILE = "nq_paper_overall_summary_final_all24.json"
OUT_CSV_FILE = "nq_paper_overall_summary_final_all24.csv"

ERROR_CASES_FILE = "nq_paper_error_cases_final_all24.csv"
MAX_ERROR_LOGS_PER_MODE = 200

MITI_CASES_FILE = "nq_mitigation_cases_final_all24.csv"
...
def generate_candidate_extractive_answers(question, context, K=10):
    spans = extract_topk_spans_by_overlap(context, question, max_words=10, topk=K)
    if not spans:
        fb = fallback_span_from_context(context, question)
...
def always_extract_correct(question, reference, ref_sentence, base_answer, context="", K=1):
    ...
    for lbl, cands in groups.items():
        ...
        # 👉 base answer 的 score（无 reference）
        base_score = 0.0

        # extractiveness bonus
        ...
        # self-consistency（越稳定越好）
        base_score += (1.0 - sc_grp) * 0.5
        ...
        for ai in cands:
            ...
            # ⭐ (NEW) NLI with context（不是 reference！）
            ent, con = detector_mnli_signal(context, ai)

            fact_score = max(ent - 1.5 * con, 0.0)
            score += 0.4 * fact_score

            # (3) self-consistency（组级）
            sc_i = selfcheck_one_answer(ai, [x for x in cands if x != ai])
            score += 0.5 * (1.0 - sc_i)

            # (4) length prior（短答案更像 NQ）
            length = len(ai.split())
            if 1 <= length <= 8:
                score += 0.2
            elif length > 15:
