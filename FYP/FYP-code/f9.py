...
# ---------- Files ----------
ANSWER_CACHE_FILE = "f_answers_cache9.csv"

OUT_JSON_FILE = "f_paper_overall_summary9.json"
OUT_CSV_FILE  = "f_paper_overall_summary9.csv"

ERROR_CASES_FILE = "f_paper_error_cases9.csv"
MAX_ERROR_LOGS_PER_MODE = 200

MITI_CASES_FILE = "f_mitigation_cases9.csv"
...
- If you are uncertain or the evidence is unclear → output EXACTLY: ABSTAIN
...
def compute_nei_and_abstain(answer_text: str):
    ...
    # 只有“没有结构输出” or 明确拒答 才算 abstain
    abstain = False

    if lbl is None:
        # 没有 LABEL → 结构错误 → 视为 abstain
        abstain = True
    elif any(p in text for p in ABSTAIN_PATTERNS):
        abstain = True
...
def detector_conditioned_correct(
    ...
    if abstain_if_still_risky and (best_ok < abstain_threshold):
        ...
        nei_ans = (
            ...
            "REASON: The evidence does not provide sufficient information to verify the claim."
        )

        nei_ok, _ = score_single_answer_full(
            question, evidence, gold_label, nei_ans, sc_override=best_group_sc
        )

        if nei_ok >= best_ok:
            return nei_ans, nei_ok, f"{action}_nei", base_ok
