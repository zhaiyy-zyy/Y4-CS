...
# ---------- Files ----------
ANSWER_CACHE_FILE = "nq_answers_cache_final_all25.csv"

OUT_JSON_FILE = "nq_paper_overall_summary_final_all25.json"
OUT_CSV_FILE = "nq_paper_overall_summary_final_all25.csv"

ERROR_CASES_FILE = "nq_paper_error_cases_final_all25.csv"
MAX_ERROR_LOGS_PER_MODE = 200

MITI_CASES_FILE = "nq_mitigation_cases_final_all25.csv"
...
def score_candidate_nq(question, context, answer, peer_answers):
    score = 0.0

    # 1) extractiveness
    if is_span_in_context(answer, context):
        score += 0.5

    # 2) question overlap
    q_words = set(normalize_answer(question).split())
    a_words = set(normalize_answer(answer).split())
    overlap = len(q_words & a_words) / (len(q_words) + 1e-6)
    score += 0.3 * overlap

    # 3) self-consistency
    sc = selfcheck_one_answer(answer, peer_answers)
    score += 0.3 * (1.0 - sc)

    # 4) local context NLI
    local_ctx = get_local_context(context, answer)
    ent, con = detector_mnli_signal(local_ctx, answer)
    score += 0.2 * max(ent - 1.5 * con, 0.0)
...
def extract_topk_spans_by_overlap(context, question, max_words=10, topk=10):
...
def _filter_candidates(question, candidates, context):
    ...
    for c in candidates:
        ...
        # extractive answer 直接保留
        if is_span_in_context(ans, context):
            kept.append(ans)
            continue

        # rewrite answer：不在context里时，放宽保留
        # 但长度别太长
        if len(ans.split()) <= 12:
            kept.append(ans)
...
- Be concise
- Prefer copying from context
- Minor rewriting allowed
...
"""
    try:
        ...
        return final_clean_output(resp["message"]["content"])
    except:
        return ""
...
def detector_conditioned_correct(
    ...
    for lbl, cands in groups.items():
        ...
        base_ok_sc, _ = score_single_answer_full(
            question, reference, ref_sentence, base_answer, sc_override=0.0
        )

        local_best_ans = base_answer
        local_best_ok = base_ok_sc

        for idx, ai in enumerate(cands):
            peer_answers = [x for j, x in enumerate(cands) if j != idx]
            sc_i = selfcheck_one_answer(ai, peer_answers)

            oki = score_candidate_nq(
                question,
                context,
                ai,
                [x for x in cands if x != ai]
            )
...
def always_extract_correct(question, reference, ref_sentence, base_answer, context="", K=1):
    ...
    for lbl, cands in groups.items():
        ...
        # base score（不使用 reference）
        base_score = 0.0

        if is_span_in_context(base_answer, context):
            base_score += 0.5

        base_len = len(base_answer.split())
        if 1 <= base_len <= 8:
            base_score += 0.2
        elif base_len > 15:
            base_score -= 0.2

        base_score += 0.3 * (1.0 - sc_grp)
        ...
        for ai in cands:
            ...
            # 1) extractiveness
            if is_span_in_context(ai, context):
                score += 0.5

            # 2) question overlap
            q_words = set(normalize_answer(question).split())
            a_words = set(normalize_answer(ai).split())
            overlap = len(q_words & a_words) / (len(q_words) + 1e-6)
            score += 0.3 * overlap

            # 3) self-consistency
            sc_i = selfcheck_one_answer(ai, [x for x in cands if x != ai])
            score += 0.3 * (1.0 - sc_i)
            ...
            # 5） local-context NLI（新加的）
            local_ctx = get_local_context(context, ai)

            ent_ctx, con_ctx = detector_mnli_signal(local_ctx, ai)

            score += 0.2 * max(ent_ctx - 1.5 * con_ctx, 0.0)
