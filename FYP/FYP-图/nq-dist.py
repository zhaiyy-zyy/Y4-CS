...
# 1️⃣ 读取数据
# ===============================
df = pd.read_csv("nq_mitigation_cases.csv")

# ⭐ 只保留最终 mitigation（关键！）
df = df[df["policy"] == "detector_conditioned_extract"].copy()
...
ax1.set_title("Reliability Score Distribution (NQ)", fontsize=13, fontweight="bold")
ax1.set_xlabel("Reliability Score")
ax1.set_ylabel("Density")
ax1.legend(frameon=False)
ax1.grid(alpha=0.3)

# ===== ② Score Shift（加 jitter 防重叠）=====
...
# ===== 总标题 =====
fig.suptitle("Natural Questions: Detection and Mitigation Analysis", fontsize=15, fontweight="bold")

# ===============================
# 7️⃣ 保存
# ===============================
plt.tight_layout()
plt.savefig("nq_combined_figure_clean.png", dpi=300, bbox_inches='tight')
plt.show()